...
function setDisabled(noProject){["startBtn","stopBtn","logsBtn","openBtn","openIdeBtn","changePathBtn","historyRefreshBtn","execRunBtn","execLatestBtn","execHistoryRefreshBtn","execTokenCopyBtn","applyConfigBtn","renameBtn","removeProjectBtn","smartProtectBtn","smartRestoreBtn","explorerRefreshBtn","savePropsBtn"].forEach(id=>{const n=$(id);if(n)n.disabled=noProject||busy})}
...
async function openIDE(){
  if(!selectedId)return;
  const ide=localStorage.getItem("riftsync_preferred_ide")||"auto";
  busy=true;
  try{
    await api("/app/open-ide",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ide})});
    const label=ide==="antigravity"?"Antigravity IDE":ide==="vscode"?"VS Code":ide==="cursor"?"Cursor":ide==="default"?"System Default":"IDE";
    toast("Project workspace opened in "+label+".","success");
  }catch(err){
    toast(err.message,"error");
  }finally{
    busy=false;
...
$("startBtn").onclick=()=>action("/app/start");$("stopBtn").onclick=()=>action("/app/stop");$("startAllBtn").onclick=startAll;$("openBtn").onclick=()=>action("/app/open-folder");$("openIdeBtn").onclick=openIDE;$("configOpenBtn").onclick=()=>action("/app/open-folder");$("changePathBtn").onclick=()=>setPanel("configPanel");
...
const ideSelect=$("preferredIdeSelect");
if(ideSelect){
  ideSelect.value=localStorage.getItem("riftsync_preferred_ide")||"auto";
  ideSelect.onchange=e=>{
    localStorage.setItem("riftsync_preferred_ide",e.target.value);
    toast("Preferred IDE updated.","info");
  };
}
